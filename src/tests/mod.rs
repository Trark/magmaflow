...
fn load_noop() {
    ...
    let ty_int = Core::OpTypeInt(OpTypeInt(ResultId(6), vec![32], vec![0]));
    let ty_vec = Core::OpTypeVector(OpTypeVector(ResultId(7), OpId(6), vec![3]));
    ...
    let instructions = vec![Core::OpCapability(OpCapability(Capability::Shader)),
                            Core::OpExtInstImport(OpExtInstImport(OpId(1), "GLSL.std.450".into())),
